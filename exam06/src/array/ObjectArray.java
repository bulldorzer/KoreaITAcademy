...
public class ObjectArray {
	...
	public ObjectArray() {
		arr = new Object[ARR_SIZE];
		count = 0;
	}
	
	public ObjectArray(int arr_size) {
//		인수로 받은 크기만큼 배열 설정해라
		arr = new Object[arr_size]; 
		count = 0;
	}
	...
	public void printAll() {
		...
		for (int i = 0; i < count; i++) {
			...
			if (obj instanceof Book) {
				Book b = (Book) obj;
				System.out.printf("제목 : %s 작가 : %s",b.getTitle(),b.getAuthor()+"\n");
			}else if (obj instanceof Music) {
				Music m = (Music) obj;
				System.out.printf("제목 : %s 가수 : %s",m.getTitle(),m.getSinger()+"\n");
			}else if (obj instanceof Movie) {
				Movie mv = (Movie) obj;
			System.out.printf("제목 : %s 감독 : %s",mv.getTitle(),mv.getDirctor()+"\n");
			}else break;
		}
	}
